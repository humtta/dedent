...
In Go, raw string literals preserve every character from the source, including
leading whitespace. When a multiline string is indented to align with
surrounding code, that indentation becomes part of the value. This package
identifies the shared indentation across lines and removes it.
...
- [`D`] accepts a string and returns a new one with the shared indentation
  removed from each line.
- [`Df`] formats the given string with [`fmt.Sprintf`] and passes the result to
  [`D`].
...
- Does not treat spaces and tabs as equivalent characters. Shared indentation
  evaluation requires byte-for-byte equivalence.
